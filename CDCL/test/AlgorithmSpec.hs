...
import           CDCL.Types (Activity (..), BoolVal (..), CDCLResult (..),
                     InterpretResult (..), Level (..), Reason (..), Tupel,
...
spec =
    describe "testing" $ do
        ...
        it "searchTupel should return 1" $
            searchTupel 3 [((Variable 1,BFalse), Decision),((Variable 2,BTrue), Decision),((Variable 3,BTrue), Decision),((Variable 4,BTrue), Decision)] `shouldBe` BTrue
        it "searchTupel should return 0" $
            searchTupel 1 [((Variable 1,BFalse), Decision),((Variable 2,BTrue), Decision),((Variable 3,BTrue), Decision),((Variable 4,BTrue), Decision)] `shouldBe` BFalse
